...
export async function getMemberStories() {
  ...
  return entry?.data?.memberStoryCollection?.items;
}

export async function getSingleMemberStory(slug) {
  const entry = await fetchGraphQL(
    `query {
            memberStoryCollection(where: {slug: "${slug}"}, limit: 1) {
                items {
                    memberName
                    description
                    image {
