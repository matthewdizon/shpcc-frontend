...
function GintongButilHistory() {
    ...
    async function fetchData() {
      if (user) {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_SERVER}/api/loans/gbl/history/` +
            user.email,
          {
            headers: {
  ...
  const loans = userApplications[0]?.gintongButilLoanApplications.map(
    (loan) => loan._id
  );
              ...
              {data?.map((loan, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                      <Link
                        href={`/dashboard/loan/gintong-butil/${loan._id}`}
...
export default GintongButilHistory;
